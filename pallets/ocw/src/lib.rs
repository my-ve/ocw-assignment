	...
	impl<T: Config> Pallet<T> {
		...
		fn fetch_price_info() -> Result<(), Error<T>> {
			...
			if let Ok(_) = lock.try_lock() {
				const DOT_HTTP_REMOTE_REQUEST: &str = "https://api.coincap.io/v2/assets/polkadot";

				log::info!("Sending request to: {}", DOT_HTTP_REMOTE_REQUEST);

				let request = rt_offchain::http::Request::get(DOT_HTTP_REMOTE_REQUEST);
